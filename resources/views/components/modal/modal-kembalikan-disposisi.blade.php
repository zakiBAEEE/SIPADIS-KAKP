<div class="flex justify-center">
    ...
    <button type="button" data-toggle="modal" data-target="#modalKembalikanDisposisi-{{ $disposisi->id }}"
        {{-- <- ID TARGET SEKARANG DINAMIS & BENAR --}}
    ...
    <div class="fixed antialiased inset-0 bg-slate-900/60 flex justify-center items-center opacity-0 pointer-events-none transition-opacity duration-300 ease-out z-[9999]"
        id="modalKembalikanDisposisi-{{ $disposisi->id }}" {{-- <- ID MODAL SEKARANG DINAMIS & BENAR --}} aria-hidden="true">

        {{-- Latar belakang untuk menutup modal saat diklik --}}
        <div class="fixed inset-0" data-dismiss="modal"></div>

        <div
            class="relative bg-white rounded-lg w-10/12 md:w-7/12 lg:w-5/12 transition-transform duration-300 ease-out scale-95 shadow-xl">
                ...
                <div class="p-6 space-y-4">
                    <div class=" w-full">
                        @include('components.base.dropdown', [
                            'label' => 'Tujuan Kembalikan Surat',
                            'value' => $daftarUserSuratDikembalikan->values()->toArray(),
                            'name' => 'ke_user_id',
                        ])
                    </div>
                    <div>
                        <label for="catatan_pengembalian-{{ $disposisi->id }}"
                        ...
                        <textarea name="catatan_pengembalian" id="catatan_pengembalian-{{ $disposisi->id }}" rows="4"
                            ...
                            required placeholder="Tuliskan alasan mengapa disposisi ini dikembalikan..."></textarea>
                    </div>
                </div>
                <div class="px-6 py-4 flex justify-end gap-x-3 bg-gray-50 border-t border-gray-200">
                    <button type="button" data-dismiss="modal"
                        class="px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</button>
                    <button type="submit"
                        class="px-4 py-2 bg-red-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-red-700">Kirim
                        Pengembalian</button>
                </div>
            </form>
        </div>
