        ...
        @if (isset($user))
            ...
            <div class="tab-content-container">
                ...
                <div id="tab-tertahan" class="tab-content">
                    <h2 class="text-xl font-semibold text-gray-800 mb-4">Surat Tertahan</h2>
                                ...
                                <table class="min-w-full  text-slate-800">
                                    <thead class="bg-slate-50 text-slate-600">
                                        <tr>
                                            ...
                                            <th class="p-3 text-sm">No. Surat</th>
                                            <th class="p-3 text-sm">Perihal</th>
                                            <th class="p-3 text-sm">Dari → Ke</th>
                                    ...
                                    <tbody>
                                        @foreach ($logRiwayat as $log)
                                            <tr class="hover:bg-slate-50 border-b">
                                                <td class="p-3 text-sm">{{ $log->created_at->format('d M Y') }}</td>
                                                <td class="p-3 text-sm">{{ $log->suratMasuk->nomor_surat ?? '-' }}</td>
                                                <td class="p-3 text-sm">{{ $log->suratMasuk->perihal ?? '-' }}</td>
                                                <td class="p-3 text-sm">{{ $log->pengirim->name ?? '-' }} →
                                                    {{ $log->penerima->name ?? '-' }}</td>
                                                <td class="p-3 text-sm">{{ $log->status ?? '-' }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
